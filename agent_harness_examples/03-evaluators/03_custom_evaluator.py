...
Two approaches to creating evaluators:
  1. Subclass CustomEvaluator (this file) — structured logging, automatic [name] prefix
  2. Implement the Evaluator protocol directly (see 04_protocol_evaluator.py)

This file demonstrates the **base class approach**.

When to use CustomEvaluator:
...
When to use the Protocol approach instead (see 04_protocol_evaluator.py):
...
-----
    ...
    2. Install dependencies and run:
        ...
        uv sync
        uv run python evaluators/03_custom_evaluator.py
"""
...
class ResponseLengthEvaluator(CustomEvaluator):
    ...
    Demonstrates:
      - Calling super().__init__(name="length_check") to set the evaluator name
      - Using self.log_info() and self.log_warning() with structured kwargs
      - Accessing context.get("session_id") for session tracking
    """
...
class KeywordEvaluator(CustomEvaluator):
    ...
    Demonstrates:
      - Using self.log_info() for successful keyword matches
      - Using self.log_warning() for missing keywords
      - Structured logging with found/missing keyword lists
    """
...
class TurnCounterEvaluator(CustomEvaluator):
    ...
    Demonstrates:
      - Maintaining state across evaluate() calls (self.count)
      - Using self.log_info() with f-string messages
      - Accessing result.success for status reporting
    """
