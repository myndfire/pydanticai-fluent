...
CIRCUIT_BREAKER_BAD_MODEL_NAME = os.getenv(
    "CIRCUIT_BREAKER_BAD_MODEL_NAME", "this-model-does-not-exist"
)
CIRCUIT_BREAKER_THRESHOLD = int(os.getenv("CIRCUIT_BREAKER_THRESHOLD", "3"))
CIRCUIT_BREAKER_TIMEOUT = int(os.getenv("CIRCUIT_BREAKER_TIMEOUT", "5"))
...
async def main():
    ...
    cb = (
        ...
        .with_threshold(CIRCUIT_BREAKER_THRESHOLD)      # open after N consecutive failures
        .with_timeout(CIRCUIT_BREAKER_TIMEOUT)          # wait Ns before half-open trial
    ...
    bad_agent = (
        ...
        .with_model(ModelConfig(
            provider=GUARDRAILS_MODEL_PROVIDER,
            model_name=CIRCUIT_BREAKER_BAD_MODEL_NAME  # will fail
        ))
    ...
    good_agent = (
        ...
        .with_circuit_breaker(
            CircuitBreakerConfig()
            .with_threshold(CIRCUIT_BREAKER_THRESHOLD)
            .with_timeout(CIRCUIT_BREAKER_TIMEOUT)
        )
    )
