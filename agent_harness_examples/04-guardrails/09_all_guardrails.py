...
-----
    ...
    2. Install dependencies and run:
        ...
        uv sync
        uv run python guardrails/09_all_guardrails.py
"""
...
ALL_GUARDRAILS_AGENT_MAX_RETRIES = int(os.getenv("ALL_GUARDRAILS_AGENT_MAX_RETRIES", "3"))
ALL_GUARDRAILS_AGENT_TIMEOUT = int(os.getenv("ALL_GUARDRAILS_AGENT_TIMEOUT", "30"))
ALL_GUARDRAILS_AGENT_BACKOFF = float(os.getenv("ALL_GUARDRAILS_AGENT_BACKOFF", "2.0"))
ALL_GUARDRAILS_TOKEN_MAX_TOTAL = int(os.getenv("ALL_GUARDRAILS_TOKEN_MAX_TOTAL", "500"))
ALL_GUARDRAILS_INPUT_COST = float(os.getenv("ALL_GUARDRAILS_INPUT_COST", "0.000003"))
ALL_GUARDRAILS_OUTPUT_COST = float(os.getenv("ALL_GUARDRAILS_OUTPUT_COST", "0.000015"))
ALL_GUARDRAILS_MAX_TOTAL_COST = float(os.getenv("ALL_GUARDRAILS_MAX_TOTAL_COST", "0.01"))
ALL_GUARDRAILS_CIRCUIT_THRESHOLD = int(os.getenv("ALL_GUARDRAILS_CIRCUIT_THRESHOLD", "5"))
ALL_GUARDRAILS_CIRCUIT_TIMEOUT = int(os.getenv("ALL_GUARDRAILS_CIRCUIT_TIMEOUT", "30"))
ALL_GUARDRAILS_AGENT2_MAX_RETRIES = int(os.getenv("ALL_GUARDRAILS_AGENT2_MAX_RETRIES", "2"))
ALL_GUARDRAILS_AGENT2_TOKEN_MAX_TOTAL = int(os.getenv("ALL_GUARDRAILS_AGENT2_TOKEN_MAX_TOTAL", "300"))
ALL_GUARDRAILS_AGENT2_MAX_TOTAL_COST = float(os.getenv("ALL_GUARDRAILS_AGENT2_MAX_TOTAL_COST", "0.005"))
ALL_GUARDRAILS_AGENT2_CIRCUIT_THRESHOLD = int(os.getenv("ALL_GUARDRAILS_AGENT2_CIRCUIT_THRESHOLD", "3"))
...
async def main():
    ...
    agent = (
        ...
        .with_agent_retries(
            AgentRetryConfig()
            .with_max_retries(ALL_GUARDRAILS_AGENT_MAX_RETRIES)
            .with_timeout(ALL_GUARDRAILS_AGENT_TIMEOUT)
            .with_backoff(ALL_GUARDRAILS_AGENT_BACKOFF)
        ...
        .with_token_limits(
            TokenLimitsConfig()
            .with_max_total_tokens(ALL_GUARDRAILS_TOKEN_MAX_TOTAL)
            .on_token_limit(on_token_limit)
        )
        # Cost limits (GPT-4o pricing)
        .with_cost_limits(
            CostLimitsConfig()
            .with_cost_per_input_token(ALL_GUARDRAILS_INPUT_COST)
            .with_cost_per_output_token(ALL_GUARDRAILS_OUTPUT_COST)
            .with_max_total_cost(ALL_GUARDRAILS_MAX_TOTAL_COST)
            .on_cost_limit(on_cost_limit)
        )
        # Circuit breaker
        .with_circuit_breaker(
            CircuitBreakerConfig()
            .with_threshold(ALL_GUARDRAILS_CIRCUIT_THRESHOLD)
            .with_timeout(ALL_GUARDRAILS_CIRCUIT_TIMEOUT)
    ...
    agent2 = (
        ManagedAgent()
        .with_model(ModelConfig(provider=GUARDRAILS_MODEL_PROVIDER, model_name=GUARDRAILS_MODEL_NAME))
        .with_agent_retries(AgentRetryConfig().with_max_retries(ALL_GUARDRAILS_AGENT2_MAX_RETRIES))
        .with_guardrails(
            content_filter=ContentFilterConfig().on_filter(content_filter),
            pii_detection=PIIDetectionConfig().on_redact(redact_pii),
            token_limits=TokenLimitsConfig().with_max_total_tokens(ALL_GUARDRAILS_AGENT2_TOKEN_MAX_TOTAL),
            cost_limits=CostLimitsConfig()
            .with_cost_per_input_token(ALL_GUARDRAILS_INPUT_COST)
            .with_cost_per_output_token(ALL_GUARDRAILS_OUTPUT_COST)
            .with_max_total_cost(ALL_GUARDRAILS_AGENT2_MAX_TOTAL_COST),
        )
        .with_circuit_breaker(CircuitBreakerConfig().with_threshold(ALL_GUARDRAILS_AGENT2_CIRCUIT_THRESHOLD))
    )
