...
async def main():
    """Run the planning loop demo.

    Setup:
        - Ollama must be running (`ollama serve`).
        - Model must be pulled (default: `ollama pull llama3.1:8b`).
        - `PLANNING_MODEL_NAME` or `MODEL_NAME` may override the model
          in `.env`.
        - `PLAN_MAX_STEPS`, `MAX_TOKENS` may be set in `.env`.
