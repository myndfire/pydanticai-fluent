...
Records are gated by `HARNESS_TELEMETRY_LEVEL` (`minimal` emits none). The
application explicitly chooses `configure_console()` for console-only harness
logs or `configure_otlp(...)` for an OpenTelemetry pipeline.
